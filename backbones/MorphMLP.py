...
class MorphFC_S(nn.Module):
    ...
    def forward(self, x):
        ...
        tmp = self.segment_dim
        # H
        h = x.transpose(3, 2).reshape(B, T, H * W // tmp, tmp, self.segment_dim, S).permute(0, 1, 2, 4, 3, 5).reshape(B,
                                                                                                                      T,
                                                                                                                      H * W // tmp,
                                                                                                                      self.segment_dim,
                                                                                                                      tmp * S)
        h = self.mlp_h(h).reshape(B, T, H * W // tmp, self.segment_dim, tmp, S).permute(0, 1, 2, 4, 3, 5).reshape(B, T,
                                                                                                                  W, H,
                                                                                                                  C).transpose(
            3, 2)
        # W
        w = x.reshape(B, T, H * W // tmp, tmp, self.segment_dim, S).permute(0, 1, 2, 4, 3, 5).reshape(B, T,
                                                                                                      H * W // tmp,
                                                                                                      self.segment_dim,
                                                                                                      tmp * S)
        w = self.mlp_w(w).reshape(B, T, H * W // tmp, self.segment_dim, tmp, S).permute(0, 1, 2, 4, 3, 5).reshape(B, T,
                                                                                                                  H, W,
                                                                                                                  C)
        # C
        c = self.mlp_c(x)
...
class MorphMLP_32_features_only(nn.Module):
    """ MorphMLP
    """

    def __init__(self, path_to_config):
        super().__init__()
        cfg = Dict(yaml.load(open(path_to_config),
    ...
    def forward_features(self, x):
        ...
        feas.append(x.permute(0, 4, 1, 2, 3))
        x = self.patch_embed2(x)
        for blk in self.blocks2:
            x = blk(x)
        feas.append(x.permute(0, 4, 1, 2, 3))
    ...
    def load_weight(self, path):
        ...
        self.load_state_dict(get_pretrained_model(path), strict=False)
